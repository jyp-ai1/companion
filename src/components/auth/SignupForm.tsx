...
import { completeReferralSignup } from "@/app/actions/growth";
...
export function SignupForm() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (refCode) {
        await completeReferralSignup(refCode);
      }
  ...
  return (
    <Card className="mx-auto w-full max-w-md">
      <h1 className="mb-2 text-center">관심사로 이어지는 동행</h1>
      <p className="mb-8 text-center text-gray-600">
        {refCode
          ? "초대를 받으셨네요. 같이 시작해 볼까요?"
          : "가입부터 첫 동행까지 약 3분이면 충분합니다."}
      </p>
